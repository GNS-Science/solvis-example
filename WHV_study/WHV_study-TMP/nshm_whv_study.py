...
def fault_system_branches(sol:solvis.CompositeSolution, rupt_ids: typing.List[int], short_name:str ='CRU'):
...
if __name__ == '__main__':
    ...
    # very off/odd, we have to try this once, otherwise the fiona setup fails (HUH ???)
    try:
        aoi_gdf = gpd.read_file(kml, driver="KML")
    except:
        pass
    ...
    whv_polygon = aoi_gdf.geometry[0]
    whv_polygon_buffered = whv_polygon.buffer(0.05)

    model = nm.get_model_version("NSHM_1.0.4")
    slt = model.source_logic_tree()
    comp = solvis.CompositeSolution.from_archive(pathlib.Path("NSHM_1.0.4_CompositeSolution.zip"), slt)
    ...
    filtered_rates.to_csv(pathlib.Path(output_folder, 'NSHM_1.0.4_WHV_study_ruptures.csv'))
    ...
    assert 0
    #branch reports
    rupt_ids = fault_system_solution.get_ruptures_intersecting(whv_polygon_buffered)
    for branch in fault_system_branches(comp, list(rupt_ids), 'CRU'):
        ...
        ruptures_with_rates_df.to_csv(pathlib.Path(output_folder, f'{fname}_ruptures.csv'), index=False)

        # build the mfd
        mfd = build_mfd(ruptures_with_rates_df, "Annual Rate")
        mfd.to_csv(pathlib.Path(output_folder, f'{fname}_mfd.csv'))

        #plot the mfd

        p = plot_mfd(mfd, title=f'Branch: {branch.values} \n weight: {round(branch.weight, 6)}' )
        p.savefig(pathlib.Path(output_folder, f'{fname}_mfd_plot.png'))
        p.close()
